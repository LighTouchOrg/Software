window.addEventListener("DOMContentLoaded", () => {
  ...
  if (RightSwipe) {
    RightSwipe.textContent = getTranslatedKey(
      "Swipe_Right_Key",
      "ArrowRight",
      lang
    );
  ...
  if (LeftSwipe) {
    LeftSwipe.textContent = getTranslatedKey(
      "Swipe_Left_Key",
      "ArrowLeft",
      lang
    );
...
function addKeybindListener(element, storageKey, lang, otherKeys = []) {
  ...
  element.addEventListener("keydown", (event) => {
    ...
    if (isKeyUsedElsewhere(key, otherKeys)) {
      ...
      if (lang == "en") showToast(`Key "${key}" already used for another key.`);
      else
        showToast(
          `La touche "${key}" est deja utilisee pour une autre action.`
        );
      return;
    } else {
